...
//          var regex = new RegExp(options.regex);
	    ...
	    if(options.replace) {
	    	...
	    	val = val.replace(new RegExp(options.regex, "g"), options.replace); // anything BUT chars ... make sure regex uses [^
		// Now only let one replace char at a time...
		val = val.replace(new RegExp("["+options.replace+"]+", "g"), options.replace);
		setTimeout(function() {
	    		input.val(val); // Must delay, so keypress isnt damaged for other validation needs
		}, 1);
		return true; 
	    } else if (!options.replace && regex.test(string)) {
              return true;
          ...
          var jquery_version_major = parseInt(jQuery.fn.jquery.split('.')[0]);
          var jquery_version_minor = parseInt(jQuery.fn.jquery.split('.')[1]);
          if (options.live) {
            if (jquery_version_major > 1 || jquery_version_minor > 7) {
